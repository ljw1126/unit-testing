...
namespace unit_testing.Chapter9.V2
{
    public class User
    {
    ...
    public class IntegrationTests : IDisposable
    {
        ...
        // 예제 9.7 ~ 8 스파이(BusSpy) 사용해서 검증 
        [Fact]
        public void Changing_email_from_corporate_to_non_corporate_spy()
        {
            // Arrange
            var db = new Database(ConnectionString);
            User user = CreateUser("user2@mycorp.com", UserType.Employee, db); // 이메일이 유니크라서 첫번째 예제와 충돌남 ..
            CreateCompany("mycorp.com", 1, db);
