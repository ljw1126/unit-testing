...
namespace unit_testing.Chapter10.TR
{
    public class User
    {
    ...
    public class IntegrationTests : IDisposable
    {
        ...
        public void Changing_email_from_corporate_to_non_corporate()
        {
            ...
            using (var transaction = new Transaction(ConnectionString))
            {
                ...
                user = CreateUser("user@mycorp.com", UserType.Employee, userRepository);
                CreateCompany("mycorp.com", 1, companyRepository);

                transaction.Commit(); // ✅ 호출해줘야 디비 반영됨
            }
    ...
    public class UserRepository
    {
        ...
        {
            using (SqlConnection connection = new SqlConnection(_transaction.ConnectionString)) // ✅ DB 트랜잭션 개방
            {
                string updateQuery = @"
                    UPDATE [dbo].[User]
